...
## 樽のtagを消す
	execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:barrel",Count:1b,tag:{display:{}}}},predicate=hd.rc:age,distance=..1] if data entity @s Item.tag.display.Name run data remove entity @s Item.tag
## ASを消す
	tag @s add HdRcThis
