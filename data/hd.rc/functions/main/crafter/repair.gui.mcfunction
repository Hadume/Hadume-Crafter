...
## データを消す
	...
	execute if entity @s[tag=!HdSctRM] if data entity @s data.Using.Recipe run data remove storage hd.rc:temp ItemsCopy[{Slot:15b}]
	execute if entity @s[tag=HdSctRM] run data remove storage hd.rc:temp ItemsCopy[{Slot:15b}]
## アイテムを返す
