#> hd.sct:main/crafter/check.recipe/anywhere/loop
#
# レシピ確認(アイテムさえあってればどこにあってもいい)
#
# @within function hd.sct:main/crafter/check.recipe/anywhere/
# @private
...
## アイテムが同じだったら、データを移しておく
	execute if score $HdSct.lib.Different HdSct.Temp matches 0 run function hd.sct:main/crafter/check.recipe/anywhere/move
## 
	execute if data storage hd.sct:temp RecipesCopy[-1].Items[-10] run function hd.sct:main/crafter/check.recipe/anywhere/loop
