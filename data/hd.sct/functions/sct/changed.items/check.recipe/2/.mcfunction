#> hd.sct:sct/changed.items/check.recipe/2/
#
...
## レシピの確認
	execute if data storage hd.sct:temp RecipesCopy[] run function hd.sct:sct/changed.items/check.recipe/2/find.recipe/loop
## 同じレシピがあったら、次の工程を実行する
	execute if score $HdSct.Same.Recipe HdSct.Temp matches 0 if data storage hd.sct:temp RecipesCopy[] run function hd.sct:sct/changed.items/check.recipe/3/set.complete
## 同じレシピが無かったら、完成アイテムSlotを変える
	execute unless data storage hd.sct:temp RecipesCopy[] run function hd.sct:sct/changed.items/check.recipe/reset.complete
