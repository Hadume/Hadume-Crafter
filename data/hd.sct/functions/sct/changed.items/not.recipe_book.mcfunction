...
## 素材アイテムを減らすか確認
	execute if data entity @e[predicate=hd.sct:storage,limit=1] data.Using.Recipe run function hd.sct:sct/changed.items/remove.material/check
## 特殊作業台のどのGUIのアイテムが変っていたら
	execute if data entity @e[predicate=hd.sct:storage,limit=1] data.Using.Recipe run scoreboard players add $HdSct.Changed HdSct.Temp 1
