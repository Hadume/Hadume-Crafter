...
## レシピが同じだったら
	execute if score $HdSct.lib.Different HdSct.Temp matches 0 unless data storage hd.sct:temp ItemsCopy[-10] unless data storage hd.sct:temp RecipesCopy[-1].Items[] run data modify storage hd.sct:temp RecipesCopy[-1].Items set from storage hd.sct:temp RecipeItemsStash
## 一時レシピアイテムを消す
