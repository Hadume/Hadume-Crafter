...
## リストの大きさを取得
	execute if data storage hd.sct: AddRecipes[-1].display[] if data storage hd.sct: AddRecipes[-1].Complete.Slot if data storage hd.sct: AddRecipes[-1].Complete.id if data storage hd.sct: AddRecipes[-1].Complete.Count run function hd.sct:recipes/add/2
## レシピが追加出来なかったら
