...
## Slot, id, Count, Rewardsがあるか確認
	execute store result score $HdSct.Elements HdSct.Temp if data storage hd.sct: AddRecipes[-1].display[].Slot
	execute if score $HdSct.Lists HdSct.Temp = $HdSct.Elements HdSct.Temp store result score $HdSct.Elements HdSct.Temp if data storage hd.sct: AddRecipes[-1].display[].id
