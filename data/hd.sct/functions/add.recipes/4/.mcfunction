#> hd.sct:add.recipes/4/
#
# レシピを追加
#
# @within function hd.sct:add.recipes/3

## displayをさくせい
	execute unless data storage hd.sct:temp AddRecipe.display run function hd.sct:add.recipes/4/display/
## Slotを消す
	execute if data storage hd.sct:temp AddRecipe.Options.Anywhere run data remove storage hd.sct:temp AddRecipe.Items[].Slot
## レシピを左上にずらす
	execute unless data storage hd.sct:temp AddRecipe.Options.Anywhere run function hd.sct:add.recipes/4/shift.slot
## レシピリストをずらす
